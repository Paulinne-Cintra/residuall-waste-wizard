...
const EditProjectForm: React.FC<EditProjectFormProps> = ({ project, onUpdate, onCancel }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (project) {
      setFormData({
        ...
        dimensionamento: project.dimensions_details || '',
        tipologia: project.project_type || '',
        status: project.status || 'planejamento',
        dataInicio: project.start_date || '',
        previsaoFinalizacao: project.planned_end_date || '',
        orcamento: project.budget || null,
        equipeResponsavel: project.responsible_team_contacts || '',
        etapasSelecionadas: [],
        materiais: [],
        documentos: [],
        observacoes: project.description_notes || ''
      });
    }
  }, [project]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updatedProject = {
        ...
        planned_end_date: formData.previsaoFinalizacao,
        budget: formData.orcamento,
        responsible_team_contacts: formData.equipeResponsavel,
        description_notes: formData.observacoes,
        updated_at: new Date().toISOString()
