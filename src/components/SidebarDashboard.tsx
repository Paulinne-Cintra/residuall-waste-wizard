...
const SidebarDashboard = () => {
  ...
  const SidebarLink = ({
    ...
    className={`${active ? "sidebar-menu-item-active" : "sidebar-menu-item"}`} onClick={() => setMobileOpen(false)}>
  ...
  const sidebarContent = <>
      <div className="flex items-center justify-between p-4 border-b border-sidebar-border">
        <Link to="/dashboard" className="flex items-center text-xl font-bold"> {/* Remover text-white daqui */}
          {!collapsed && <>
               ...
               */}
              <img alt="Logo Residuall" style={{
            maxHeight: '32px',
            width: 'auto'
          }} src="/lovable-uploads/d4826481-6237-4186-8ced-b0b927c45423.png" className="h-8 mr-2 block object-scale-down" />
      ...
      <div className="p-4 border-t border-sidebar-border">
        <Link to="/" className="sidebar-menu-item" onClick={() => setMobileOpen(false)}>
  ...
  return <>
    ...
    className="md:hidden fixed bottom-4 right-4 z-30 bg-residuall-green text-white p-3 rounded-full shadow-lg">
      ...
      <div className={`fixed inset-0 bg-black bg-opacity-50 z-40 md:hidden transition-opacity ${mobileOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`} onClick={toggleMobileSidebar}></div>

      {/* Sidebar */}
      <aside className={`fixed left-0 top-0 bottom-0 z-50 flex flex-col md:relative md:z-0
